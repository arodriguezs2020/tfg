...
public interface HabitacionRepository extends JpaRepository<Habitacion, Integer>{
	@Query(value = "SELECT h FROM HABITACIONES h WHERE NOT EXISTS (SELECT null FROM RESERVA r WHERE r.habitacion.id = h.id AND :fecha2 <= r.entrada AND :fecha1 >= r.salida)",
			nativeQuery = true)
